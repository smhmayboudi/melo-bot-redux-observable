...
const operate: (message: IStateMessageQuery) => void = (message: IStateMessageQuery): void => {
  const mongoClientObservable:
  ...
  if (message.message !== undefined) {
    ...
    switch (message.message.text) {
      // TODO: TEST
      case "/getChatMember":
        store.dispatch(actions.getChatMember.query({
          query: {
            chat_id: "@melodio",
      ...
      // TODO: TEST
      case "/literate":
        store.dispatch(actions.literate.query({
          query: "HI",
        }));
        break;
      // TODO: TEST
      case "/sendAudio":
        store.dispatch(actions.sendAudio.query({
          query:
          {
      ...
      // TODO: TEST
      case "/sendVideo":
        store.dispatch(actions.sendVideo.query({
          query:
          {
      ...
      case "/youtubeDownload":
        store.dispatch(actions.youtubeDownload.query({
          query: "/dl_RTB5eGxxZlhmRVk"
            .replace("/dl_", "")
            .trim(),
        }));
        break;
      ...
      case "/youtubeSearchList":
        store.dispatch(actions.youtubeSearchList.query({
          query: {
            key: env.KEY,
            maxResults: 10,
            part: "id,snippet",
      ...
      case "/youtubeVideoList":
        store.dispatch(actions.youtubeVideoList.query({
          query: {
            chart: "mostPopular",
            key: env.KEY,
            maxResults: 10,
      ...
      default:
        if (message.message.text !== undefined) {
          ...
          } else if (message.message.text.includes(`/${texts.commandMostPopular}`)) {
            store.dispatch(actions.youtubeVideoList.query({
              query: {
                chart: "mostPopular",
                key: env.KEY,
          ...
          } else if (message.message.text.includes(`/${texts.commandDownload}${texts.commandSeparator}`)) {
            store.dispatch(actions.youtubeDownload.query({
              query: message.message.text
                .replace(`/${texts.commandDownload}${texts.commandSeparator}`, "")
                .trim(),
            }));
          } else if (message.message.text.includes(`/${texts.commandRelatedToVideoId}${texts.commandSeparator}`)) {
            store.dispatch(actions.youtubeSearchList.query({
              query: {
                key: env.KEY,
                relatedToVideoId: message.message.text
                  .replace(`/${texts.commandRelatedToVideoId}${texts.commandSeparator}`, "")
                  .trim(),
              },
            }));
          } else {
