  ...
  async (videoId: string): Promise<any> =>
    new Promise((resolve: (value?: any | PromiseLike<any>) => void, reject: (reason?: any) => void): void => {
      ...
      const parseVideoInfo:
        (videoInfo: string) => IVideoInfo[] =
        (videoInfo: string): IVideoInfo[] => {

          const rxFmtList: RegExp = /fmt_list=([\]\[!"#$%'()*+,.\/:;<=>?@\^_`{|}~-\w]*)/;
          const fmtListmap: string = unescape((videoInfo.match(rxFmtList) as RegExpMatchArray)[1]);

          const rxFmtListNumG: RegExp = /\d+/g;
          const fmtListNums: RegExpMatchArray = fmtListmap.match(rxFmtListNumG) as RegExpMatchArray;
          ...
          const urlmap: string = unescape((videoInfo.match(rxUrlMap) as RegExpMatchArray)[1]);
          ...
          durs = map(durs, (s: string): string => (s.match(rxDur) as RegExpMatchArray)[1]);

          const rxItag: RegExp = /itag=([\]\[!"#$%'()*+,.\/:;<=>?@\^_`{|}~-\w]*)/;
          let itags: RegExpMatchArray = urlmap.match(rxUrlG) as RegExpMatchArray;
          itags = map(itags, unescape);
          itags = map(itags, (s: string): string => (s.match(rxItag) as RegExpMatchArray)[1]);
          ...
          urls = map(urls, (s: string): string => (s.match(rxUrl) as RegExpMatchArray)[1]);
      ...
      const downloadVideo:
        (videoInfos: IVideoInfo[]) => void =
        (videoInfos: IVideoInfo[]): void => {

          const videoInfo: IVideoInfo = videoInfos
            .filter((value: IVideoInfo): boolean => value.mime === "video/mp4")
            .sort((a: IVideoInfo, b: IVideoInfo) => a.fmtList.width - b.fmtList.width)
          [0];

          const thumbDownload:
            (vi: IVideoInfo) => Promise<any> =
            async (vi: IVideoInfo): Promise<any> =>
                    ...
                    https
                      .request(
                        ...
                        (response: http.IncomingMessage): void => {
                          appDebug("thumbnail response.statusCode", response.statusCode);
                          const stream: fs.WriteStream = fs.createWriteStream(thumbPath);
                          response
                            .on("end", (): void => {
                  ...
                  if (err === null && stats.size > 0) {
                    ...
                    https
                      .request(
                        vi.url,
                        (response: http.IncomingMessage): void => {
                          appDebug("video response.statusCode", response.statusCode);
                          const stream: fs.WriteStream = fs.createWriteStream(videoPath);
                          response
                            .on("end", (): void => {
