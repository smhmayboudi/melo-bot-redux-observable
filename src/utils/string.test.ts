...
import { caption, decode, encode, pathThumb, pathVideo, transformSearchList, transformVideoList } from "./string";
...
describe("string utils", (): void => {
    ...
    test("should handle pathThumb", (): void => {
        expect(pathThumb(query))
            .toContain(`${result}.jpg`);
    });

    test("should handle pathVideo", (): void => {
        expect(pathVideo(query))
            .toContain(`${result}.mp4`);
    });

    test("should handle transformSearchList items length", (): void => {
        const q: string = "";
        const items: youtube_v3.Schema$SearchResult[] = [];
        expect(transformSearchList(items, q))
            .toEqual(texts.messageNoResult);
    });
    ...
    test("should handle transformVideoList items length", (): void => {
        ...
        expect(transformVideoList(items))
            .toEqual(texts.messageNoResult);
    });
