...
const sendAudio:
  ...
  (
    ...
    const transform:
      (query: IStateSendAudioQuery) => FormData =
      (query: IStateSendAudioQuery): FormData => {
    ...
    const actionObservable:
      (action: IActionSendAudio) => Observable<IActionSendAudio> =
      (action: IActionSendAudio): Observable<IActionSendAudio> => {
        if (botToken === undefined) {
          return of(actions.sendAudio.error({
        ...
        return requestsUploadObservable(
          ...
          .pipe(
            map((response: IResponse): IActionSendAudio => {
            ...
            catchError((error: any) => of(actions.sendAudio.error({
              error,
            }))),
          );
      };
