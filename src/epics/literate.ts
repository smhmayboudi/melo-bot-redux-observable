...
const literate:
  ...
  (
    ...
    const actionObservable:
      (action: IActionLiterate) => Observable<IActionLiterate> =
      (action: IActionLiterate): Observable<IActionLiterate> => {
        if (requestObservable === undefined) {
          return of(actions.literate.error({
        ...
        return requestObservable(
          ...
          .pipe(
            map((result: IStateLiterate): IActionLiterate =>
              actions.literate.result(result),
            ),
            catchError((error: any) => of(actions.literate.error({
              error,
            }))),
          );
      };
